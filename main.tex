...
\end{tabular}
\end{table}
Figure 1 and 2 show the measured efficiency.  in the $N_{jet} $ and $ N_{b-jet}$ plane for each year. Each row consists of two-dimensional efficiency maps, and each individual plot corresponds to a specific $H_T$ bin. There is a clear dependence on $N_{jet} $, $ N_{b-jet}$, and $H_T$ which justifies the necessity of corrections depending on the variables. The bottom row in each 2D plot corresponds to $N_{jet} = 6$ and shows low efficiencies due to the requirement of $N_{jet} \geq 6$ in trigger paths. This region is used only for the validation of the background estimation methods. Details will be discussed in Section 5. 
\begin{figure}[!t]
    \centering
...
\section{MC and Data plot for resolved top tagger input variables}
\label{sec:restop_input}
...
\begin{figure}[H]
    ...
    \whiledo{\row < 4}{% 4 rows
        ...
        \whiledo{\col < 4}{%
            ...
            \begin{minipage}[b]{0.24\textwidth}
                \centering
                \includegraphics[width=\textwidth]{plots/restop_var/plots_2018/2018_\arabic{img}.png}
            \end{minipage}%
            \advance\col by 1
        }
        \vskip 0.5cm
    ...
    \captionof{figure}{MC and data comparison plots for different input variables for 2018. A selection of $N_{\text{jets}} \geq 6$, $N_{\text{bjets}} \geq 2$ and $H_T > 700$ GeV is applied, mirroring the selection for our control and signal regions. Triggers used in the main analysis are also applied to the data.}
\end{figure}
...
\begin{figure}[H]
    ...
    \whiledo{\row < 4}{% 4 rows
        ...
        \whiledo{\col < 4}{%
            ...
            \begin{minipage}[b]{0.24\textwidth}
                \centering
                \includegraphics[width=\textwidth]{plots/restop_var/plots_2023/2023_\arabic{img}.png}
            \end{minipage}%
            \advance\col by 1
        }
        \vskip 0.5cm
    ...
    \captionof{figure}{MC and data comparison plots for different input variables for 2023. A selection of $N_{\text{jets}} \geq 6$, $N_{\text{bjets}} \geq 2$ and $H_T > 700$ GeV is applied, mirroring the selection for our control and signal regions. Triggers used in the main analysis are also applied to the data.}
\end{figure}

\clearpage

\section{Trigger object review}
\label{sec:restop_input}
The studies performed during the trigger object review are summarized in this appendix.
Since the primary goal of the review is to confirm the reliability of the trigger efficiency measurement and application methods in this analysis, UL2018 data and MC samples are used as a representative example.
These results are used to validate the trigger strategy employed in the main analysis.
